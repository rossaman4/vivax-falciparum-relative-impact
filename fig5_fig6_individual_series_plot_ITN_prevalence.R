...
# fig5_fig6_individual_series_plot_prevalence.r
#####
...
par(mfrow=c(3,5))
for (val in x)
{plot((pr_numb_vivax_LM/(pr_numb_falciparum_LM+pr_numb_vivax_LM))~time_zero, type='n', data=data_new_prev, main=first_row_study$plot_title[first_row_study$study_number_new==val] , xlab='Time [months]', ylab= 'proportion vivax', ylim=c(0:1), bty='n', yaxt='n', xaxt='n', xlim=c(0,40), cex.main=0.9) 
   ifelse((val==1|val==36|val==136), axis(2,labels=TRUE, las=1), axis(2,labels=NA))
   ifelse((val==136|val==151|val==99|val==183|val==154), axis(1,labels=TRUE, las=1), axis(1,labels=NA))
  ...
  text(2.5,1,fave_cols$letter[fave_cols$number==val],cex=1.2)
...
legend('bottomright',legend=c('<100 cases', '100-199 cases', '200-499 cases', '500-999 cases', '>999 cases'), col=c('navy'), pch=c(16), pt.cex=c(1.5*0.75, 1.5*1, 1.5*1.25, 1.5*1.5, 1.5*1.75), cex=0.9)
...
fave_cols<-cbind.data.frame(colour= c("#9c496c",
                                      ...
                                      "#9c496c",
                                      "#5ab74d",
                                      "#8e5bc8"), number =c(39,  40,173, 174, 177, 178, 179, 180,169, 356, 184, 189, 185, 338, 339, 340, 341, 342, 343, 344, 345, 346, 347,  41, 135,21),letter=c('a)','b)','c)','d)','e)','f)','g)','h)','i)','j)','k)','l)','m)','n)','o)','p)','q)','r)','s)','t)','u)','v)','w)','x)','y)','z)'))
...
par(mfrow=c(4,7))
for (val in x)
{plot((pr_numb_vivax_LM/(pr_numb_falciparum_LM+pr_numb_vivax_LM))~time_zero, type='n', data=data_new_prev, main=first_row_study$plot_title[first_row_study$study_number_new==val] , xlab='Time [months]', ylab= 'proportion vivax', ylim=c(0:1),xlim=c(0,60), bty='n', yaxt='n', xaxt='n', cex.main=0.8) 
  ifelse((val==39|val==180|val==339|val==346), axis(2,labels=TRUE, las=1), axis(2,labels=NA))
