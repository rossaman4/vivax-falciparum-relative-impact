...
tiff("individual_spag_IRS_prev.tiff", width = 7, height = 5, units = 'in', res = 700, pointsize=8)
#quartz()
...
for (val in x)
{plot((pr_numb_vivax_LM/(pr_numb_falciparum_LM+pr_numb_vivax_LM))~time_zero, type='n', data=data_new_prev, main=first_row_study$plot_title[first_row_study$study_number_new==val] , xlab='Time [months]', ylab= 'proportion vivax', ylim=c(0:1), bty='n', yaxt='n', xaxt='n', xlim=c(0,50), cex.main=0.9) 
  ifelse((val==120), axis(2,labels=TRUE, las=1), axis(2,labels=NA))
         axis(1,labels=NA)
  points((pr_numb_vivax_LM/(pr_numb_vivax_LM+pr_numb_falciparum_LM))~time_zero, 
...
for (val in x)
{plot((pr_numb_vivax_LM/(pr_numb_falciparum_LM+pr_numb_vivax_LM))~time_zero, type='n', data=data_new_prev, main=first_row_study$plot_title[first_row_study$study_number_new==val] , xlab='Time [months]', ylab= 'proportion vivax', ylim=c(0:1), bty='n', yaxt='n', xaxt='n', xlim=c(0,50), cex.main=0.9) 
  ifelse((val==126|val==133), axis(2,labels=TRUE, las=1), axis(2,labels=NA))
  ifelse((val==152|val==134|val==133|val==132|val==131), axis(1,labels=TRUE, las=1), axis(1,labels=NA))
  points((pr_numb_vivax_LM/(pr_numb_vivax_LM+pr_numb_falciparum_LM))~time_zero, 
...
legend('topleft',legend=c('<100 cases', '100-199 cases', '200-499 cases', '500-999 cases', '>999 cases'), col=c('navy'), pch=c(16), pt.cex=c(2*0.75, 2*1, 2*1.25, 2*1.5, 2*1.75), cex=1.5)
