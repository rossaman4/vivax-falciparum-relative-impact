...
par(mar=c(1,1,1.5,1))
par(mfrow=c(1,2))
plot((case_numbers_vivax/(case_numbers_falciparum+case_numbers_vivax))~time_incidence,type='n', data=data_new_inc, main=expression('First time LLIN') , xlab='Time [months]', ylab=expression('Proportion of cases attributed to ' *italic(P.vivax)* ''), ylim=c(0:1),xlim=c(0,60), bty='n', las=1)
for (val in x)
...
plot((case_numbers_vivax/(case_numbers_falciparum+case_numbers_vivax))~time_incidence,type='n', data=data_new_inc, main=expression('Repeated time ITN') , xlab='Time [months]', ylab=expression('Proportion of cases attributed to ' *italic(P.vivax)* ''),ylim=c(0:1),xlim=c(0,60), bty='n', las=1)
for (val in x)
{points((incidence_rate_vivax/(incidence_rate_falciparum+incidence_rate_vivax))~time_incidence, 
...
mtext(text='time since ITN distribution in months',side=1, line=2, cex=1, outer=TRUE)
mtext(text=expression('proportion of cases that are ' *italic(P.vivax)* ''),side=2, line=2, cex=1, outer=TRUE)
legend('bottomright',legend=c('<200 cases', '200-499 cases', '500-999 cases', '1000-1999 cases', '>1999 cases'), col=c('black'), pch=c(16), pt.cex=c(0.75, 1, 1.25, 1.5, 1.75), cex=0.9)
