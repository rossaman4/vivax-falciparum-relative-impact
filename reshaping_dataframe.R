...
#figuring out time spans for implementation and for data collection
...
sum(histo_inc$density[1:6])
sum(histo_inc$density[1:12])

data_new_inc_times<-as.numeric(data_new_inc$date_end_incidence-data_new_inc$date_start_incidence)/(365.25/12)
summary(data_new_inc_times)
hist(data_new_inc_times,breaks=seq(0,70,by=1))
histo_inc<-hist(data_new_inc_times,breaks=seq(0,70,by=1))
sum(histo_inc$density[1])
sum(histo_inc$density[1:12])

first_row_study<-data_new_prev[!duplicated(data_new_prev$study_number_new),]
data_new_prev_times<-as.numeric(first_row_study$implementation_end-first_row_study$implementation_start)/(365.25/12)
summary(data_new_prev_times)
hist(data_new_prev_times,breaks=seq(0,40,by=1))
histo_prev<-hist(data_new_prev_times,breaks=seq(0,40,by=1))
sum(histo_prev$density[1])
sum(histo_prev$density[1:12])

data_new_prev_times<-as.numeric(data_new_prev$date_end_prevalence-data_new_prev$date_start_prevalence)/(365.25/12)
summary(data_new_prev_times)
hist(data_new_prev_times,breaks=seq(0,40,by=1))
histo_prev<-hist(data_new_prev_times,breaks=seq(0,40,by=1))
sum(histo_prev$density[1])
